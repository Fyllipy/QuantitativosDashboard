...
var ourRequest = new XMLHttpRequest();
    ...
    ourRequest.onload = function (){
        var ans;
            ...
            var data = dadosJSON.wall;
            var dataWindow = dadosJSON.windows;
...
var btn = document.getElementById("wall");
...
function wallForm(id){
  ...
  for(var i = 0;i < 4; i++){
    ...
    var nivel = document.createTextNode("Altura Desconectada ")
      itemName[0].appendChild(nivel);
    
    var alturaPeitoril = document.createTextNode("Comprimento ")
      itemName[1].appendChild(alturaPeitoril);
    
    var alturaExtremidade = document.createTextNode("Volume ")
      itemName[2].appendChild(alturaExtremidade);

    var alturaExtremidade2 = document.createTextNode("Area ")
    itemName[3].appendChild(alturaExtremidade2);
    ...
    var nivelValue = document.createTextNode(data[id].Alturadesconectada);
    itemValue[0].appendChild(nivelValue);
  
    var alturaPeitorilValue = document.createTextNode(data[id].Comprimento);
    itemValue[1].appendChild(alturaPeitorilValue);
    
    var alturaExtremidadeValue = document.createTextNode(data[id].Volume.toString());
    itemValue[2].appendChild(alturaExtremidadeValue);

    var alturaExtremidadeValue2 = document.createTextNode(data[id].area.toString());
    itemValue[3].appendChild(alturaExtremidadeValue2);
}
...
function windowForm(id){
  ...
  for(var i = 0;i < 3; i++){
    ...
    var nivelValue = document.createTextNode(dataWindow[id].Nixedvel);
    itemValue[0].appendChild(nivelValue);
  
    var alturaPeitorilValue = document.createTextNode(dataWindow[id].Alturadopeitoril);
    itemValue[1].appendChild(alturaPeitorilValue);
  
    var alturaExtremidadeValue = document.createTextNode(dataWindow[id].Alturadaextremidade);
    itemValue[2].appendChild(alturaExtremidadeValue);
}
...
// Get the button that opens the modal
var btnWindow = document.getElementById("window");
...
btnWindow.onclick = function() {
  ...
  var wall = document.createElement("h2");
  wall.setAttribute('id', 'headerEraser');
  var walltext = document.createTextNode("Propriedades das Janelas");
  wall.appendChild(walltext);
  modalHeader.appendChild(wall);
}
    }
